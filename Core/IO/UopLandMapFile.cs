...
public sealed class UopLandMapFile : ILandMapFile
{
    private readonly FileStream _stream;
    private readonly UopIndexEntry[] _entries;

    public MapProfile Profile { get; }

    public UopLandMapFile(string filePath, FileAccess access)
    {
        ...
        if (!UopIndexReader.TryRead(filePath, out var indexData, out var error) || indexData == null)
        {
            throw new InvalidOperationException(error ?? "Failed to read UOP index.");
        }

        _entries = indexData.Entries.ToArray();
    }

    public void ReadCell(int x, int y, out ushort tileId, out sbyte z)
    {
        ValidateCoordinates(x, y);
        var offset = CalculatePhysicalOffset(GetLogicalCellOffset(x, y));
        ...
        {
            throw new EndOfStreamException("Unexpected EOF while reading UOP map cell.");
        }
    ...
    public void ReadLandBlockTiles(int blockX, int blockY, Span<ushort> tileIds)
    {
        ...
        var logicalBlockOffset = GetLogicalBlockOffset(blockX, blockY) + 4; // skip block header
        var physicalOffset = CalculatePhysicalOffset(logicalBlockOffset);
        ...
        if (read != 192)
        {
            throw new EndOfStreamException("Unexpected EOF while reading UOP land block.");
        }
    ...
    private long GetLogicalCellOffset(int x, int y)
    {
        var blockX = x / 8;
        var blockY = y / 8;
        var offX = x % 8;
        ...
        return GetLogicalBlockOffset(blockX, blockY) + 4 + (offY * 8 + offX) * 3;
    }

    private long GetLogicalBlockOffset(int blockX, int blockY)
    {
        return (long)(blockX * Profile.HeightBlocks + blockY) * 196;
    }

    private long CalculatePhysicalOffset(long logicalOffset)
    {
        long logicalPosition = 0;

        foreach (var entry in _entries)
        {
            var nextPosition = logicalPosition + entry.Length;
            if (logicalOffset < nextPosition)
            {
                return entry.Offset + (logicalOffset - logicalPosition);
            }

            logicalPosition = nextPosition;
        }

        throw new InvalidOperationException("Logical UOP offset exceeds available entries.");
    }
